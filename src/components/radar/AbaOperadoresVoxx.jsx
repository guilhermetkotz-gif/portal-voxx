...
export default function AbaOperadoresVoxx({ mensagens = [], grupos = [], clientes = [] }) {
  ...
  const [apenasForaSLA, setApenasForaSLA] = useState(false);
  ...
  const { data: performanceData, isLoading } = useQuery({
    queryKey: ['perfOperadores', periodoObj.inicio, periodoObj.fim, filtroClassificacao, apenasCriticos, apenasPendentes, apenasForaSLA, filtroAtendente],
    queryFn: () => base44.functions.invoke('calcularPerformanceOperadoresVoxx', {
      periodoInicio: periodoObj.inicio,
      periodoFim: periodoObj.fim,
      classificacao: filtroClassificacao !== 'todas' ? filtroClassificacao : null,
      apenasCriticos,
      apenasPendentes,
      apenasForaSLA,
      remetente_telefone: filtroAtendente || null,
    }),
    staleTime: 30 * 1000,
    refetchInterval: 60 * 1000,
  });

  const operadores = performanceData?.data?.operadores || [];
  const cards = performanceData?.data?.cards || {};
  const remetentesNaoCadastrados = performanceData?.data?.remetentes_nao_cadastrados || 0;

  // Realtime: invalidar ao receber novas mensagens ou avaliações
  React.useEffect(() => {
    const unsub1 = base44.entities.WhatsappMensagem.subscribe(() => {
      queryClient.invalidateQueries({ queryKey: ['perfOperadores'] });
    });
    const unsub2 = base44.entities.WhatsappAvaliacaoMensagemVoxx.subscribe(() => {
      queryClient.invalidateQueries({ queryKey: ['perfOperadores'] });
    });
    const unsub3 = base44.entities.WhatsappRemetenteVoxx.subscribe(() => {
      queryClient.invalidateQueries({ queryKey: ['perfOperadores'] });
    });
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-lg px-3 py-2.5 flex items-center gap-2 flex-wrap">
        ...
        <Button
          size="sm"
          variant={apenasCriticos ? 'default' : 'outline'}
          onClick={() => { setApenasCriticos(!apenasCriticos); setApenasPendentes(false); setApenasForaSLA(false); }}
          className={`h-7 text-[10px] ${apenasCriticos ? 'bg-red-600 hover:bg-red-500 text-white' : 'border-slate-700 text-slate-400'}`}
        >
          Só críticos
        </Button>
        <Button
          ...
          onClick={() => { setApenasPendentes(!apenasPendentes); setApenasCriticos(false); setApenasForaSLA(false); }}
          className={`h-7 text-[10px] ${apenasPendentes ? 'bg-amber-600 hover:bg-amber-500 text-white' : 'border-slate-700 text-slate-400'}`}
        >
          Só pendentes
        </Button>
        <Button
          size="sm"
          variant={apenasForaSLA ? 'default' : 'outline'}
          onClick={() => { setApenasForaSLA(!apenasForaSLA); setApenasCriticos(false); setApenasPendentes(false); }}
          className={`h-7 text-[10px] ${apenasForaSLA ? 'bg-orange-600 hover:bg-orange-500 text-white' : 'border-slate-700 text-slate-400'}`}
        >
          Fora do SLA
        </Button>
            ...
            <tbody>
              {isLoading ? (
                ...
                <tr><td colSpan={15} className="px-4 py-10 text-center text-slate-500">
                  {remetentesNaoCadastrados > 0
                    ? 'Nenhum atendente VOXX cadastrado com mensagens no período.'
                    : cards.atendentes_ativos === 0
                      ? 'Cadastre remetentes VOXX para iniciar a avaliação por atendente.'
                      : cards.total_mensagens_avaliadas === 0
                        ? 'Existem mensagens VOXX aguardando avaliação de qualidade.'
