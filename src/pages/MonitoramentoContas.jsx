...
export default function MonitoramentoContas({ user }) {
    ...
    const radarData = React.useMemo(() => {
        ...
        // Debug: Ver quais contas não tem cliente
        ...
        return radarMetaData.map(radar => {
            ...
            if (!cliente) {
                contasSemCliente.push(radar.account_name);
                console.log('⚠️ CONTA SEM CLIENTE:', radar.account_name);
            }
        ...
        console.log(`Total de contas SEM CLIENTE: ${contasSemCliente.length}`);
        if (contasSemCliente.length > 0) {
            console.log('Contas sem cliente:', contasSemCliente);
        }
        
        // Retornar TODAS as contas, mesmo sem cliente cadastrado
        return radarMetaData.map(radar => {
            const cliente = clientesMap.get(radar.account_name);
            const conta = accounts.find(a => a.account_name === radar.account_name);
            ...
            const frequenciaOntem = radar.frequencia_ontem || 0;
            const frequencia7d = radar.frequencia7d_7d || 0;
